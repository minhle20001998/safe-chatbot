...
import {
  generateChatbotLogEntry,
  generateUserLogEntry,
  LOG_FILE_PATH,
  logChatbotResponse,
  logUserMessage,
} from './logger';
...
describe('Logger test', () => {
  ...
  it('should log chat bot response to log file', () => {
    ...
    const logFileContent = fs.readFileSync(LOG_FILE_PATH, 'utf-8');
    expect(logFileContent).toMatch(
      generateChatbotLogEntry(userID[0], CHATBOT_MESSAGES.VERIFIED_MESSAGE_RESPONSE, date),
    );
  });
});
